...
export function buildShareUrl(config: GameConfig): string {
  const payload = encodeURIComponent(JSON.stringify({ config } satisfies SharePayload));
...
export function copyShareUrl(config: GameConfig): Promise<void> {
